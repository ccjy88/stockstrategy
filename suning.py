...
def appointment():
    ...
    if issucess==False:
        logger.info('预约失败, errorcode={} {}'.format(errorcode,errormsg))
    else:
        logger.info('issucess={}, errorcode={} {}'.format(issucess,errorcode,errormsg))
    if issucess:
        sendweixin('预约成功', errormsg)
...
def nowBuy():
    ...
    if issucess==False:
        logger.info('购买失败, errorcode={} {}'.format(errorcode,errormsg))
    else:
        logger.info('issucess={}, errorcode={} {}'.format(issucess,errorcode,errormsg))
...
class Orderlist(object):
    ...
    def isLoginok(self):
        ...
        inputs = xdata.xpath('//input[@name="receiverAddr_checkAdd"]')
        #logger.info(len(inputs))
        return len(inputs)>0 #如果登录成功，应该有多个input
...
def buildappointmenturl():
    ...
    logger.info('产品ID={} 价格={} 预订ID={}'.format(partNumber,salenetprice,bookActionID))
